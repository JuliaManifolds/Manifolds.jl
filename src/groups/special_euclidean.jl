...
function affine_matrix(::GT, ::Identity{<:SemidirectProductOperation}) where {n,GT<:SpecialEuclidean{n}}
...
function translate_diff!(G::SpecialEuclidean, Y, ::Identity{<:SemidirectProductOperation}, q, X, ::RightAction)
    copyto!(G, Y, X)
    @inbounds _padvector!(G, Y)
    return Y
end
function translate_diff!(G::SpecialEuclidean, Y, p, ::Identity{<:SemidirectProductOperation}, X, ::RightAction)
    np, hp = submanifold_components(G, p)
    nX, hX = submanifold_components(G, X)
    nY, hY = submanifold_components(G, Y)
    hY .= hp' * hX * hp
