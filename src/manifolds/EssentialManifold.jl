...
denotes the matrix representation of the cross product operator. For more details see TronDaniilidis2017(@cite).
...
````
where $d(⋅,⋅)$ is the distance function in $SO(3)$TronDaniilidis2017(@cite).
"""
...
````
where $\tilde X$ is the horizontal lift of $X$TronDaniilidis2017(@cite).
"""
...
details see TronDaniilidis2017(@cite).
"""
...
For more details see Algorithm 1 in TronDaniilidis2017(@cite).
"""
...
and its proof, as well as Lemma 1 in TronDaniilidis2017(@cite).
"""
...
This function computes the derivatives of each term $f_i, i=1,2,$ at discontinuity point `t_break`. For more details see TronDaniilidis2017(@cite).
"""
...
f(t) = f_1 + f_2, \quad f_i = \frac{1}{2} θ^2_i(t), \quad θ_i(t)=d(R_{p_i},R_z(t)R_{b_i}) \text{ for } i=1,2,
````
in the interval $[$`t_low`, `t_high`$]$ using Newton's method. For more details see TronDaniilidis2017(@cite).
"""
...
Return the manifold dimension of the [`EssentialManifold`](@ref), which is `5`TronDaniilidis2017(@cite).
"""
...
pose of camera $i$ $g_i = (R_i,T'_i) ∈ \text{SE}(3)$ and $R_0 ∈ \text{SO}(3)$ such that $R_0(T'_2-T'_1) = e_z$ TronDaniilidis2017(@cite).
"""
