...
Equip a `AbstractManifold` explicitly with a [`AbstractMetric`](@ref) `G`.
...
Generate the `AbstractManifold` `M` as a manifold with the [`AbstractMetric`](@ref) `G`.
"""
...
On the `AbstractManifold` `M` with implicitly given metric ``g_1``
...
in the tangent space at `p` on the `AbstractManifold` `M` given with respect to the
...
`AbstractManifold` `M` equipped with [`AbstractMetric`](@ref) `G` to a cotangent by
computing
...
of the tangent space at `p` on the `AbstractManifold` `M` with respect
...
`AbstractManifold` `M` using the [`AbstractMetric`](@ref) `G`. If `G` is the default
...
respect to the [`AbstractBasis`](@ref) `B` on the `AbstractManifold` `M`.
...
Copute the logarithmic map on the `AbstractManifold` `M` equipped with the [`AbstractMetric`](@ref) `G`.
...
`AbstractManifold` `M` equipped with [`AbstractMetric`](@ref) `G` to a tangent by
computing
