...
@recipe function f(
    ...
    shpere_surface = true,
    sphere_color = RGBA(0.8,0.8,0.8,1.0),
    wireframe_color = RGBA(0.0,0.0,0.2,0.4),
    lat_samples=33,
    lon_samples=33,
    geodesic_interpolation = -1,
) where {T}
    φ_range = range(0,stop=2*π,length=lon_samples)
    λ_range = range(0,stop=π,length=lat_samples)
    ...
    if shpere_surface
        @series begin
            seriestype := :surface
            seriescolor := fill(sphere_color, lat_samples, lon_samples)
            x,y,z
        end
    end
    if sphere_wireframe
    ...
    if geodesic_interpolation < 0
        ...
        length(pts) > 0 && push!(lpts,last(pts)) # add last end point
...
@recipe function f(
    ...
    tickfontcolor --> RGBA(1.0, 1.0, 1.0, 1.0)
    seriestype := :quiver
    quiver := ([v[1] for v ∈ vecs], [v[2] for v ∈ vecs], [v[3] for v ∈ vecs])
    return [p[1] for p ∈ pts], [p[2] for p ∈ pts], [p[3] for p ∈ pts]
end
