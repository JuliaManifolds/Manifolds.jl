...
Compute the inner product on the [`CholeskySpace`](@ref) `M` at `p` for the tangent vectors `X` and `Y`.
Here `p` is a lower triangular matrix with positive diagonal and `X`, `Y` are lower
triangular matrices with arbitrary diagonal. The formula reads
