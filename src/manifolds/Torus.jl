...
@doc raw"""
    check_tangent_vector(M::Torus{n}, p, X)
...
"""
function check_tangent_vector(M::Torus{N}, p, X; kwargs...) where {N}
    if length(p) != N
        return DomainError(
            length(p),
    ...
    return check_tangent_vector(PowerManifold(M.manifold, N), p, X; kwargs...)
end
