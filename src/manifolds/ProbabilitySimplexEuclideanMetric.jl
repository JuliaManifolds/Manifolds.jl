...
When `vector_at` is `nothing`, return a random (uniform) point `x` on the [`ProbabilitySimplex`](@ref) `M` with the Euclidean metric.
This is done by normalizing independent exponential draws to unit sum, see [Devroye:1986](@cite),
Theorems 2.1 and 2.2 on p. 207 and 208, respectively.
