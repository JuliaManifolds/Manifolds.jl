@doc raw"""
    ProbabilitySimplex{n,closed} <: AbstractDecoratorManifold{𝔽}
...
function Base.show(io::IO, ::ProbabilitySimplex{n,closed}) where {n,closed}
    return print(io, "ProbabilitySimplex($(n); closed=$closed)")
end
