...
The tangent space at $p$, $T_p\mathcal \operatorname{SPS}_k(n)$ also represented
matrices $Y\in 𝔽^{n × k}$ reads as

 ````math
 T_p\operatorname{SPS}_k(n)(n,k) = \bigl\{
 X ∈ 𝔽^{n × n}\,|\,X = qY^{\mathrm{H}} + Yq^{\mathrm{H}}
 \text{ i.e. }  X = X^{\mathrm{H}
 \bigr\}
 ````
Note that the metric used yields a non-complete manifold. The metric was used
in[^JourneeBachAbsilSepulchre2010][^MassartAbsil2020]:
...
[^MassartAbsil2020]:
    ...
    > doi: [10.1137/18m1231389](10.1137/18m1231389),
    > arXiv: []()
"""
...
@doc raw"""
    check_manifold_point(M::SymmetricPositiveSemidefiniteFixedRank{n,𝔽}, p; kwargs...)

Check whether `p` is a valid manifold point on the [`SymmetricPositiveSemidefiniteFixedRank`](@ref) `M`, i.e.
whether `p` is a symmetric matrix of size `(n,n)` with values from the corresponding
[`AbstractNumbers`](@ref) `𝔽`.

The tolerance for the symmetry of `p` can be set using `kwargs...`.
"""
function check_manifold_point(M::SymmetricPositiveSemidefiniteFixedRank{n,k,𝔽}, p; kwargs...) where {n,k,𝔽}
    mpv =
