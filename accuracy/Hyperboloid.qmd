...
```{julia}
d_comp = [distance(M, pb, q) for q in qbs]
...
plot!(fig, lengths, abs.(d_comp .- d_1), color=indigo, label="1 - acosh-based")
plot!(fig, lengths, abs.(d_comp .- d_2), color=teal, label="2 - sinh-based")
```
