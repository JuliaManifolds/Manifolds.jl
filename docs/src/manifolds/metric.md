...
The [`MetricManifold`](@extref) decorator allows to introduce further metrics for a manifold by acting as a wrapper.
...
When wrapping a manifold `M` with a [`MetricManifold`](@extref) together with the default [`metric`](@extref)`(M)`,
...
To avoid overhead, one [`AbstractMetric`](@extref `ManifoldsBase.AbstractMetric`) can then be marked as being the default, i.e. the one that is used, when no [`MetricManifold`](@extref) decorator is present.
This avoids reimplementation of the first existing metric, access to the metric-dependent functions that were implemented using the undecorated manifold, as well as the transparent fallback of the corresponding [`MetricManifold`](@extref) with default metric to the undecorated implementations.
