...
out_file = joinpath(@__DIR__,"..","docs","src","assets","images","projection_illustration.png")
n = 100
...
push!(tp, Plot(plaint_opts_a, Coordinates(Tuple.([[.0, .0],]))))

tangent_line2 = [C_to_2D(p - 0.8*X), C_to_2D(p + 1.1*X)]
push!(tp, Plot(plain_opts, Coordinates(Tuple.(tangent_line2))))

projection_opts2 = @pgf {"no markers", "densely dotted", mark_size = projection_marker_size, color = projection_color2}
projection_line2 = [C_to_2D(q), C_to_2D(s1)]
push!(tp, Plot(projection_opts2, Coordinates(Tuple.(projection_line2))))
projection_opts2_a = @pgf {"only marks", mark_size = projection_marker_size, color = projection_color2}
...
projection_opts3 = @pgf {"no markers", "densely dotted", mark_size = projection_marker_size, color = projection_color3}
projection_line3 = [C_to_2D(q), C_to_2D(p+s2)]
push!(tp, Plot(projection_opts3, Coordinates(Tuple.(projection_line3))))
projection_opts3_a = @pgf {"only marks", mark_size = projection_marker_size, color = projection_color3}
push!(tp, Plot(projection_opts3_a, Coordinates(Tuple.([C_to_2D(p+s2),]))))

push!(tp, raw"\node[label ={[label distance=.05cm]below:{\color{gray}$q$}}] at (axis cs:"*"$(real(q)),$(imag(q))) {};")
push!(tp, raw"\node[label ={[label distance=.05cm]left:{\color{gray}$p$}}] at (axis cs:"*"$(real(p)),$(imag(p))) {};")
# push!(tp, raw"\node[label ={[label distance=.05cm]right:{$s_1=\mathrm{proj}_{\mathcal C}(q_2)=$\texttt{project(M,q2)}}}] at (axis cs:"*"$(real(s1)),$(imag(s1))) {};")
push!(tp, raw"\node[label ={[label distance=.05cm]right:{\color{gray}$s_1=$\texttt{project(M,q)$\in\mathcal C$}}}] at (axis cs:"*"$(real(s1)),$(imag(s1))) {};")
push!(tp, raw"\node[label ={[label distance=.05cm]0:{\color{gray}$s_2=$\texttt{project(M,p,q)}$\in T_{p}\mathcal C$}}] at (axis cs:"*"$(real(p+s2)),$(imag(p+s2))) {};")
