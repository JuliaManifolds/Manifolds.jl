...
out_file = joinpath(@__DIR__, "..","docs","src","assets","images","retraction_illustration.png")
n = 100
...
tangent_opts = @pgf {"no markers", "->", color = tangent_color, line_width = tangent_linewidth}
...
projection_opts = @pgf {"no markers", "densely dotted", color = projection_color, line_width = base_linewidth}
projection_line = [C_to_2D(p+X), C_to_2D(qP)]
push!(tp, Plot(projection_opts, Coordinates(Tuple.(projection_line))))
projection_opts_a = @pgf {"only marks", mark_size = projection_marker_size, color = projection_color}
push!(tp, Plot(projection_opts_a, Coordinates(Tuple.([C_to_2D(qP),]))))


push!(tp, raw"\node[label ={[label distance=.05cm]above:{\color{gray}$p$}}] at (axis cs:"*"$(real(p)),$(imag(p))) {};")
push!(tp, raw"\node[label ={[label distance=.1cm]above:{\color{gray}$X\in T_p\mathcal C$}}] at (axis cs:"*"$(real(p+X/2)),$(imag(p+X/2))) {};")
push!(tp, raw"\node[label ={[label distance=.05cm]left:{\color{gray}$q'=\mathrm{retr}^{\mathrm{proj}}_pX=\mathrm{proj}_{\mathcal C}(p+X)$}}] at (axis cs:"*"$(real(qP)),$(imag(qP))) {};")
push!(tp, raw"\node[label ={[label distance=.05cm]below:{\color{gray}$q=\exp_pX$}}] at (axis cs:"*"$(real(qE)),$(imag(qE))) {};")
