...
basis_color = dark_mode ? RGB(0.67, 0.67, 0.67) : RGB(0.33, 0.33, 0.33)
point_color = vector_color
inner_prod_color = dark_mode ? RGB(0.33, 0.33, 0.33) : RGB(0.67, 0.67, 0.67)
logo_colors = [(77, 100, 174), (57, 151, 79), (202, 60, 50), (146, 89, 163)]
rgb_logo_colors = map(x -> RGB(x ./ 255...), logo_colors)
rgb_logo_colors2 = map(x -> RGB( (dark_mode ? 1.2 : 0.8) .* x ./ 255...), logo_colors)
...
out_file_prefix = dark_mode ? "logo_dark" : "logo"
out_file_ext = ".pdf"
...
patch_colors2 = rgb_logo_colors2[2:end]
...
# vectors in tangent spaces along geodesic
