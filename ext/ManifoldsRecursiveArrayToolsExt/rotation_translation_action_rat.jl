...
function inverse_apply(
        ::RotationTranslationActionOnVector{LeftAction}, a::ArrayPartition, p,
    )
    return a.x[2] \ (p - a.x[1])
end
"""
...
"""
function inverse_apply(
        ::RotationTranslationActionOnVector{RightAction}, a::ArrayPartition, p,
    )
    return a.x[2] * p + a.x[1]
end
...
"""
    apply_diff(
        ::RotationTranslationActionOnVector{LeftAction}, a::ArrayPartition, p, X,
    )
...
"""
    apply_diff(
        ::RotationTranslationActionOnVector{RightAction}, a::ArrayPartition, p, X,
    )
...
function apply_diff!(
        ::RotationTranslationActionOnVector{LeftAction}, Y, a::ArrayPartition, p, X,
    )
    mul!(Y, a.x[2], X)
    return Y
end
function apply_diff!(
        ::RotationTranslationActionOnVector{RightAction}, Y, a::ArrayPartition, p, X,
    )
    Y .= a.x[2] \ X
    return Y
end
...
function apply_diff_group!(
        ::RotationTranslationActionOnVector{LeftAction}, Y, ::SpecialEuclideanIdentity, X::ArrayPartition, p,
    )
    Y .= X.x[2] * p
    return Y
end
...
function optimal_alignment(
        A::LeftColumnwiseSpecialEuclideanAction{<:AbstractManifold, <:SpecialEuclidean}, p, q,
    )
    ...
    Ostar = optimal_alignment(
        ColumnwiseMultiplicationAction(A.manifold, SpecialOrthogonal(N)), p_moved, q,
    )
