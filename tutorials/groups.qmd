...
`StaticArrays` can be used to speed up some operations.
...
Let's say we want to define a manifold point `p_i` some rotation θ from the [`identity`](@ref) reference rotation `p0` (another point on the manifold that we will use as reference)
...
```{julia}
X_ = hat(G, IG, xθi)              # specific definition to Lie groups
xXi = get_vector(G, p0, xθi, B)   # generalized definition beyond Lie groups
println(xXi)
