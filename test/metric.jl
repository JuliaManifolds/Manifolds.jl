...
@testset "Metric decorator" begin
    ...
    Manifolds.basis(M::BaseManifold{N},x,::ArbitraryOrthonormalBasis) where {N} = ( [(Matrix(I, N, N)[:,i]) for i in 1:N], zeros(N))
    ...
    @test basis(M,x,ArbitraryOrthonormalBasis()) == basis(MM2,x,ArbitraryOrthonormalBasis())
    @test_throws ErrorException basis(MM,x,ArbitraryOrthonormalBasis())
