...
Manifolds.get_basis(::BaseManifold{N},x,::OrthonormalBasis) where {N} = ( [(Matrix(I, N, N)[:,i]) for i in 1:N], zeros(N))
...
@testset "Metrics" begin
    ...
    @testset "Metric decorator" begin
        ...
        @test get_basis(M,x,OrthonormalBasis()) == get_basis(MM2,x,OrthonormalBasis())
        @test_throws ErrorException get_basis(MM,x,OrthonormalBasis())
