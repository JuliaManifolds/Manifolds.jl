...
@testset "The Segre manifold" begin
    ...
    @testset "exp maps to the manifold." begin
        for M in Ms
            p = rand(M)
    ...
    @testset "Geodesics are unit speed." begin
        for M in Ms
            p = rand(M)
            ...
            geodesic_speed =
                finite_difference(t -> distance(M, p, exp(M, p, t * v)), 0.5 * rand(), 1e-5)
            @test(isapprox(geodesic_speed, 1.0; rtol=1e-6))
        end
    end

    @testset "Geodesics are minimizing." begin
        for M in Ms
            p = rand(M)
            v = rand(M, vector_at=p)
            v = v / norm(M, p, v)
    ...
    @testset "log is inverse of exp." begin
        for M in Ms
            ...
            function m(a, b)
                return sqrt(
                    sum([
                        distance(Sphere(n - 1), x, y)^2 for
                        (n, x, y) in zip(V, a[2:end], b[2:end])
                    ]),
                )
            end
            q = rand(M)
    ...
    @testset "get_coordinates is left and right inverse of get_vector. " begin
        for M in Ms
            p = rand(M)
            v = rand(M, vector_at=p)
    ...
    @testset "sectional_curvature is compatible with riemann_tensor." begin
        for M in Ms
            p = rand(M)
            v = rand(M, vector_at=p)
    ...
    @testset "Sectional curvature is difference between circumference and 2 pi r for small circles" begin
        for M in Ms
            p = rand(M)
            ...
            r = 1e-2
            ps = [
                exp(M, p, r * (cos(theta) * u + sin(theta) * v)) for
